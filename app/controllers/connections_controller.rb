class ConnectionsController < ApplicationController
  ...
  def create
    ...
    if @connection.valid?
      @notification =
        Notification.create(
          sender_id: connection_params[:mentee_id],
          recipient_id: connection_params[:mentor_id],
          text: 'mentorship request'
        )
      serialized_data =
      ...
      render json: { connection: ConnectionSerializer.new(@connection) },
             status: :created
    else
      render json: { error: 'failed to create connection' },
  ...
  def update
    @connection =
      Connection.find_by(
        mentor_id: connection_params[:mentor_id],
        mentee_id: connection_params[:mentee_id]
      )
    ...
    if @connection.valid?
      ...
      @accept_notification =
        Notification.create(
          sender_id: connection_params[:mentor_id],
          recipient_id: connection_params[:mentee_id],
          text: 'mentorship accepted'
        )
      serialized_data =
        ActiveModelSerializers::Adapter::Json.new(
      ...
      render json: { error: 'failed to update connection' },
             status: :not_acceptable
    end
  end
